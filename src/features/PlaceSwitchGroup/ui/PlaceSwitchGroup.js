...
export const PlaceSwitchGroup = ({
  ...
  return `
    <div class="placeSwitchGroup ${extraClasses.join(" ")}" ${getGeneratedAttrs(extraAttrs)}>
      <div class="main__panelControl" data-js-filter="marks">
        <input class="input-style" placeholder="Введите адрес" type="text" name="search" data-js-filter-item="search" data-js-filter-parent-name="marks"/>
        ${switchElements}
      </div>
    </div>
  `;
